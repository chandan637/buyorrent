...
class UserService {
  ...
  public async findUserByIdAndSub(_id: string, sub: string): Promise<User> {
    const findUser: User = await this.users.findOne({ _id, sub }).lean().exec();
    if (!findUser) throw new HttpException(409, "You're not user");

    return findUser;
  }

  // Insert will only happen for first time google login
  public async upsertUserForOAuth(userData: CreateUserDto): Promise<User> {
